...
public:
	inline void initial(ModelParams& model, HyperParams& opts, AlignedMemoryPool* mem) {
		...
		for (int i = 0; i < seq_size; i++){
			for (int idx = 0; idx < word_inputs[i].size(); idx++) {
				if (i == seq_size - 1){
					...
					word_inputs[i][idx].init(model.words.nDim, opts.dropOut, mem);
					word_inputs[i][idx].setParam(&model.history_words);
				}
			}
		}
		...
		concat_local_context.init(model.words.nDim + opts.rnnhiddensize * 2, -1, mem);
		...
		padding.init(model.words.nDim, -1, mem);
	}
		...
		for (int i = 0; i<seq_size; i++){
			...
			if (i == seq_size - 1) {
				for (int idx = 0; idx < wordnum; idx++) {
					//input
					word_inputs[1][idx].forward(this, feature.words[idx]);
				}
				//windowlized
				word_window.forward(this, getPNodes(word_inputs[1], wordnum));
				...
				for (int idx = 0; idx < wordnum; idx++) {
					//input
					word_inputs[0][idx].forward(this, feature.words[idx]);
				}
				context_gated_pooling.forward(this, getPNodes(word_inputs[0], wordnum));
			}
		}
